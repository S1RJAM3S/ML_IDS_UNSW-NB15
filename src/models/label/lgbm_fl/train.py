...
class FLLGBMTrainer(LabelTrainer):
    ...
    def train(self, name: str = 'lgbm_fl'):
        ...
        alpha_low = round(max(0.10, alpha_mid - 0.25), 2)
        alpha_high = round(min(0.90, alpha_mid + 0.25), 2)
        ...
        def obj(trial):
            ...
            self.gamma = gamma
            self.alpha = alpha
            _, mean_recall = self._cv_loop(X_arr, y_arr)
            self.logger.info(f"Trial {trial.number} - gamma = {gamma}, alpha = {alpha} - OOF Recall = {mean_recall}")
            return mean_recall
        ...
        self.logger.info(f"Best: gamma = {self.gamma}, alpha = {self.alpha} - OOF Recall = {study.best_value}")
